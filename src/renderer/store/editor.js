...
const actions = {
  ...
  FORMAT_LINK_CLICK(_, { data, dirname }) {
  ...
  SHOW_IMAGE_DELETION_URL(_, deletionUrl) {
    notice
      .notify({
      ...
      .then(() => {
        window.api.clipboard.writeText(deletionUrl)
      })
  },
  ...
  LISTEN_FOR_SAVE_AS({ state, rootState }) {
    window.api.ipc.on('mt::editor-ask-file-save-as', () => {
      ...
      const defaultPath = getRootFolderFromState(rootState)
      if (id) {
        window.api.ipc.send('mt::response-file-save-as', {
          id,
          filename,
          pathname,
          markdown,
  ...
  LISTEN_FOR_SET_PATHNAME({ commit, dispatch, state }) {
    window.api.ipc.on('mt::set-pathname', (fileInfo) => {
      const { tabs } = state
    ...
    window.api.ipc.on('mt::tab-save-failure', (tabId, msg) => {
      const { tabs } = state
      const tab = tabs.find((t) => t.id === tabId)
      if (!tab) {
        notice.notify({
  ...
  LISTEN_FOR_CLOSE({ state }) {
    window.api.ipc.on('mt::ask-for-close', () => {
      ...
      if (unsavedFiles.length) {
        window.api.ipc.send('mt::close-window-confirm', unsavedFiles)
      } else {
        window.api.ipc.send('mt::close-window')
      }
    })
  },
  ...
  ASK_FOR_SAVE_ALL({ commit, dispatch, state }, closeTabs) {
    ...
    if (closeTabs) {
      if (unsavedFiles.length) {
        commit(
          'CLOSE_TABS',
          tabs.filter((f) => f.isSaved).map((f) => f.id),
        )
        window.api.ipc.send('mt::save-and-close-tabs', unsavedFiles)
      } else {
        commit(
          'CLOSE_TABS',
          tabs.map((f) => f.id),
  ...
  LISTEN_FOR_MOVE_TO({ state, rootState }) {
    window.api.ipc.on('mt::editor-move-file', () => {
      ...
      if (!pathname) {
        ...
        window.api.ipc.send('mt::response-file-move-to', { id, pathname })
      }
    })
  },
  ...
  RENAME({ state }, newFilename) {
    ...
    if (typeof filename === 'string' && filename !== newFilename) {
      ...
      window.api.ipc.send('mt::rename', { id, pathname, newPathname })
    }
  },
  ...
  LISTEN_FOR_BOOTSTRAP_WINDOW({ commit, dispatch, rootState }) {
    ...
    window.api.ipc.on('mt::bootstrap-editor', (config) => {
      const { addBlankTab, markdownList, lineEnding, sideBarVisibility, tabBarVisibility, sourceCodeModeEnabled } =
        config
  ...
  LISTEN_FOR_NEW_TAB({ dispatch }) {
    window.api.ipc.on('mt::open-new-tab', (markdownDocument, options = {}, selected = true) => {
      if (markdownDocument) {
    ...
    window.api.ipc.on('mt::new-untitled-tab', (selected = true, markdown = '') => {
      // Create a blank tab
  ...
  LISTEN_FOR_TAB_CYCLE({ dispatch }) {
    ...
    window.api.ipc.on('mt::tabs-cycle-right', () => {
  ...
  LISTEN_FOR_CONTENT_CHANGE({ commit, dispatch, state, rootState }, { id, markdown, wordCount, cursor, history, toc }) {
    ...
    const { id: currentId, filename, pathname, markdown: oldMarkdown, trimTrailingNewline } = state.currentFile
    const { listToc } = state
  ...
  EXPORT({ state }, { type, content, pageOptions }) {
    ...
    window.api.ipc.send('mt::response-export', {
      type,
      title,
      content,
      filename,
  ...
  LINTEN_FOR_SET_LINE_ENDING({ commit, dispatch, state }) {
    window.api.ipc.on('mt::set-line-ending', (lineEnding) => {
  ...
  LINTEN_FOR_SET_ENCODING({ commit, state }) {
    window.api.ipc.on('mt::set-file-encoding', (encodingName) => {
  ...
  LINTEN_FOR_SET_FINAL_NEWLINE({ commit, state }) {
    window.api.ipc.on('mt::set-final-newline', (value) => {
  ...
  LISTEN_FOR_FILE_CHANGE({ commit, state, rootState }) {
    window.api.ipc.on('mt::update-file', ({ type, change }) => {
  ...
  LISTEN_WINDOW_ZOOM({ dispatch, rootState }) {
    window.api.ipc.on('mt::window-zoom', (zoomFactor) => {
      ...
      window.api.webFrame.setZoomFactor(zoomFactor)
    })
  },
