...
const cli = () => {
  ...
  if (process.env.NODE_ENV === 'development') {
    ...
    argv = ['--user-data-dir', path.join(getPath('appData'), 'vien-dev')]
  }

  const args = parseArgs(argv, true)
  if (args['--help']) {
    write(`Usage: vien [commands] [path ...]
...
`)
  ...
  if (args['--version']) {
    writeLine(`Vien: ${global.MARKTEXT_VERSION_STRING}`)
  ...
  if (!args['--user-data-dir']) {
    const portablePath = path.join(app.getAppPath(), '..', '..', 'vien-user-data')
