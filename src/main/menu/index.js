...
class AppMenu {
  ...
  _listenForIpcMain() {
    ...
    ipcMain.on('mt::update-line-ending-menu', (_e, windowId, lineEnding) => {
      this.updateLineEndingMenu(windowId, lineEnding)
    })
    ipcMain.on('mt::update-format-menu', (_e, windowId, formats) => {
    ...
    ipcMain.on('mt::update-sidebar-menu', (_e, windowId, value) => {
      if (!this.has(windowId)) {
        log.error(`UpdateApplicationMenu: Cannot find window menu for window id ${windowId}.`)
        return
      }
    ...
    ipcMain.on('mt::view-layout-changed', (_e, windowId, viewSettings) => {
      if (!this.has(windowId)) {
        log.error(`UpdateApplicationMenu: Cannot find window menu for window id ${windowId}.`)
        return
      }
    ...
    ipcMain.on('mt::editor-selection-changed', (_e, windowId, changes) => {
      if (!this.has(windowId)) {
        log.error(`UpdateApplicationMenu: Cannot find window menu for window id ${windowId}.`)
        return
      }
