 ...
 */
const getStyles = (element: HTMLElement): CSSStyleDeclaration | null => {
  if (!element) {
    return null
  }
 ...
 */
const resizeCodeBlockLineNumber = (element: HTMLElement) => {
  // FIXME: Heavy performance issues with this function, please see #1648.

  const codeStyles = getStyles(element)
  const whiteSpace = codeStyles?.['white-space' as unknown as number] as unknown as string | undefined
  ...
  if (whiteSpace === 'pre' || whiteSpace === 'pre-wrap' || whiteSpace === 'pre-line') {
    ...
    let lineNumberSizer = element.querySelector('.line-numbers-sizer') as HTMLElement | null
    ...
    codeLines.forEach((line: string, lineNumber: number) => {
