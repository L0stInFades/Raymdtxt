 ...
 */
export const offsetToWordCursor = (lineCursor: Cursor, left: number, right: number): Cursor => {
  // Deep clone cursor start and end
  const start = deepClone(lineCursor.start) as CursorPosition
  const end = deepClone(lineCursor.end) as CursorPosition
  start.offset = left
  end.offset = right
  return { start, end }
}
...
export const validateLineCursor = (selection: {
  ...
  if (
    !selection ||
    !selection.start ||
    !('offset' in selection.start) ||
    !selection.end ||
    !('offset' in selection.end)
  ) {
    return false
  }
 ...
 */
export const extractWord = (text: string, offset: number) => {
