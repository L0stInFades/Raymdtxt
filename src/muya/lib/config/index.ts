...
export const DEFAULT_TURNDOWN_CONFIG = Object.freeze({
  ...
  blankReplacement(_content: string, node: HTMLElement & { isBlock?: boolean }, _options: unknown) {
    if (node?.classList.contains('ag-soft-line-break')) {
      return LINE_BREAK
    } else if (node?.classList.contains('ag-hard-line-break')) {
      return `  ${LINE_BREAK}`
    } else if (node?.classList.contains('ag-hard-line-break-sapce')) {
      return ''
    } else {
      return node.isBlock ? '\n\n' : ''
    }
