...
class Keyboard {
  ...
  recordIsComposed() {
    ...
    const handler = (event: CompositionEvent) => {
  ...
  dispatchEditorState() {
    ...
    const changeHandler = (event: Event) => {
      ...
      if ((event.target as Element)?.closest('[contenteditable=false]')) {
        return
      }
      ...
      switch (event.code) {
        ...
        case EVENT_KEYS.Space: {
          if (contentState.selectedImage) {
            const { token } = contentState.selectedImage as { token: { src?: string; attrs?: { src?: string } } }
  ...
  keyupBinding() {
    ...
    const handler = (event: KeyboardEvent) => {
      ...
      if (
        ...
        const reference = getParagraphReference(emojiNode as HTMLElement, paragraph.id)
      ...
      if (!this.isComposed) {
        ...
        if (
          ...
          contentState.cursor = { anchor, focus, start: start ?? anchor, end: end ?? focus }
          if (needRender) {
