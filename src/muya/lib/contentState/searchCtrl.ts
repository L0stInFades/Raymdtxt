...
const matchString = (text: string, value: string, options: Record<string, boolean>) => {
...
const searchCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.buildRegexValue = (match: { match: string; subMatches: string[] }, value: string) => {
  ...
  ContentState.prototype.find = function (action: string /* prev next */) {
  ...
  ContentState.prototype.search = function (value: string, opt = {}) {
    const matches: Array<{ key: string; start: number; end: number; match: string; subMatches: string[] }> = []
