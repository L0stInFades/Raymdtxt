...
const tableBlockCtrl = (ContentState: { prototype: IContentState }) => {
  ...
  ContentState.prototype.createTable = function (tableChecker: { rows: number; columns: number }) {
  ...
  ContentState.prototype.tableToolBarClick = function (type: string) {
    const {
      start: { key },
    } = this.cursor
    ...
    switch (type) {
      ...
      case 'right': {
        ...
        table.children.forEach((rowContainer: Block) => {
          rowContainer.children.forEach((row: Block) => {
      ...
      case 'table': {
        ...
        const handler = (row: number, column: number) => {
          ...
          let i: number
          if (column > (oldColumn as number)) {
            for (i = (oldColumn as number) + 1; i <= column; i++) {
              ...
              bodyRows.forEach((bodyRow: Block) => {
          ...
          } else if (column < (oldColumn as number)) {
  ...
  ContentState.prototype.editTable = function (
    ...
    if (target === 'row') {
      ...
      if (action === 'insert') {
        ;[...thead.children, ...tbody.children].forEach((tableRow) => {
          ...
          tableRow.children.forEach((cell: Block, i: number) => {
            cell.column = i
          })
        })
        cursorBlock =
          location === 'left' ? this.getPreSibling(cellBlock)!.children[0] : this.getNextSibling(cellBlock)!.children[0]
        ...
        ;[...thead.children, ...tbody.children].forEach((tableRow) => {
          ...
          if (removeCell === cellBlock) {
            cursorBlock = this.findNextBlockInLocation(block) ?? undefined
          }
    ...
    if (cursorBlock) {
      ...
      this.cursor = { start: start!, end: end! }
    }
  ...
  ContentState.prototype.getTableBlock = function (): Block | null {
    const { start, end } = this.cursor
    const startBlock = this.getBlock(start.key)!
    const endBlock = this.getBlock(end.key)!
    const startParents = this.getParents(startBlock)
    const endParents = this.getParents(endBlock)
    const affiliation = startParents.filter((p: Block) => endParents.includes(p))

    if (affiliation.length) {
      const figure = affiliation.find((p: Block) => p.type === 'figure')
      return figure ?? null
    }
    return null
  }

  ContentState.prototype.tableBlockUpdate = function (block: Block) {
    const { type } = block
