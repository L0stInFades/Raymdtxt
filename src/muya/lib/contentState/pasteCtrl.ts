...
const pasteCtrl = (ContentState: { prototype: IContentState }) => {
  // check paste type: `MERGE` or `NEWLINE`
  ContentState.prototype.checkPasteType = function (start: Block, fragment: Block) {
  ...
  ContentState.prototype.checkCopyType = (html: string, rawText: string) => {
    let type = 'normal'
    if (!html && rawText) {
      type = 'copyAsMarkdown'
  ...
  ContentState.prototype.standardizeHTML = async (rawHtml: string) => {
  ...
  ContentState.prototype.docPasteHandler = async function (event: ClipboardEvent) {
  ...
  ContentState.prototype.pasteHandler = async function (event: ClipboardEvent, type = 'normal', rawText: string | null = null, rawHtml: string | null = null) {
    ...
    if (startBlock!.functionType === 'cellContent') {
      ...
      if (isOneCellSelected) {
        ...
        offset += start.offset
        startBlock!.text =
          startBlock!.text.substring(0, start.offset) + pendingText + startBlock!.text.substring(end.offset)
      }
    ...
    const stateFragments =
      type === 'pasteAsPlainText' || copyType === 'copyAsMarkdown' ? this.markdownToState(text) : this.html2State(html)
    ...
    const getLastBlock = (blocks: Block[]) => {
    ...
    switch (pasteType) {
      ...
      case 'NEWLINE': {
        let target: Block = parent!
        stateFragments.forEach((block: Block) => {
          this.insertAfter(block, target)
          target = block
        })
