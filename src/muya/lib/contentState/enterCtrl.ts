...
const enterCtrl = (ContentState: { prototype: IContentState }) => {
  ...
  ContentState.prototype.chopBlock = function (block: Block) {
  ...
  ContentState.prototype.createBlockLi = function (paragraphInListItem: Block | null) {
  ...
  ContentState.prototype.createTaskItemBlock = function (paragraphInListItem: Block | null, checked = false) {
  ...
  ContentState.prototype.enterInEmptyParagraph = function (block: Block) {
    ...
    if (parent && /ul|ol|blockquote/.test(parent.type)) {
      ...
      if (parent.listItemType === 'task') {
        ...
        newBlock = this.createTaskItemBlock(null, checked ?? false)
      } else {
  ...
  ContentState.prototype.docEnterHandler = function (event: KeyboardEvent) {
  ...
  ContentState.prototype.enterHandler = function (event: KeyboardEvent) {
    ...
    const getFirstBlockInNextRow = (row: Block) => {
    ...
    const getParagraphBlock = (block: Block) => {
      if (block.type === 'li') {
    ...
    if (htmlNeedFocus) {
      ...
      const match = /^[^\n]+\n[^\n]*/.exec(text)
      offset = match?.[0] ? match[0].length : 0
    }
