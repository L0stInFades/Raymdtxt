...
const backspaceCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.checkBackspaceCase = function (this: IContentState) {
  ...
  ContentState.prototype.docBackspaceHandler = function (this: IContentState, event: KeyboardEvent) {
  ...
  ContentState.prototype.backspaceHandler = function (this: IContentState, event: KeyboardEvent) {
    ...
    if (/th/.test(maybeCell!.type) && start.offset === 0 && !maybeCell!.preSibling) {
      if (
        ...
        const figureBlock = this.closest(startBlock!, 'figure')
    ...
    if ((node as HTMLElement).classList.contains('ag-image-container')) {
      const imageWrapper = node!.parentNode as HTMLElement
      const imageInfo = getImageInfo(imageWrapper)
      if (start.offset === (imageInfo.token as Token).range.end) {
        event.preventDefault()
        event.stopPropagation()
    ...
    const tableHasContent = (table: Block) => {
    ...
    if (
      ...
      if (!block!.nextSibling) {
        ...
        switch ((preBlock as Block).functionType) {
          case 'fencecode':
          case 'indentcode':
          case 'frontmatter':
            referenceBlock = preBlock as Block
            break
          case 'multiplemath':
          case 'flowchart':
          case 'mermaid':
          ...
          case 'vega-lite':
          case 'html':
            referenceBlock = this.getParent(preBlock as Block)
            break
        }
      ...
      if ((!preBlock || preBlock.functionType !== 'cellContent') && !hasContent) {
        ...
        delete (figure as Block).functionType
        ;(figure as Block).children = []
        this.appendChild(figure as Block, paragraphContent)
        ;(figure as Block).text = ''
        ;(figure as Block).type = 'p'
