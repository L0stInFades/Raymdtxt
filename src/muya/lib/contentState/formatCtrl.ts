...
const getOffset = (offset: number, { range: { start, end }, type, tag, anchor, alt }: Token) => {
  ...
  switch (type) {
    ...
    case 'link': {
      const MARKER_LEN = 1
      const anchorStr = (anchor as string) ?? ''
      if (dis < MARKER_LEN) return 0
      if (dis >= MARKER_LEN && dis <= MARKER_LEN + anchorStr.length) return -1
      if (dis > MARKER_LEN + anchorStr.length) return anchorStr.length - dis
      break
    }
    case 'image': {
      const MARKER_LEN = 1
      ...
      if (dis >= MARKER_LEN * 2 && dis <= MARKER_LEN * 2 + altStr.length) return -2
      if (dis > MARKER_LEN * 2 + altStr.length) return altStr.length - dis
      break
    }
  }
}
...
const clearFormat = (
  ...
  if (end) {
    ...
    end.delata = (end.delata ?? 0) + (delataEnd ?? 0)
  }
  switch (token.type) {
    case 'strong':
    case 'del':
...
const checkTokenIsInlineFormat = (token: Token) => {
  ...
  if (type === 'html_tag' && /^(?:u|sub|sup|mark)$/i.test(tag as string)) return true
  return false
}

const formatCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.selectionFormats = function ({ start, end }: Cursor = selection.getCursorRange()) {
    if (!start || !end) {
  ...
  ContentState.prototype.format = function (type: string) {
      ...
      if (type === 'clear') {
        ...
        if (type === 'image') {
          ...
          requestAnimationFrame(() => {
            ...
            if (startNode) {
              const imageWrapper = (startNode as Element).closest('.ag-inline-image')
              if (imageWrapper?.classList.contains('ag-empty-image')) {
