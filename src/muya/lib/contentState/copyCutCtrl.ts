...
const copyCutCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.docCutHandler = function (event: ClipboardEvent) {
  ...
  ContentState.prototype.cutHandler = function () {
    ...
    if (selectedImage) {
      const { key, token } = selectedImage as { key: string; token: { raw: string; [key: string]: unknown } }
      this.deleteImage({
        key,
        token,
      })
  ...
  ContentState.prototype.docCopyHandler = function (event: ClipboardEvent) {
    const { selectedTableCells } = this
    if (selectedTableCells) {
      event.preventDefault()
      ...
      for (i = 0; i < row; i++) {
        const rowWrapper = []
        for (j = 0; j < column; j++) {
          const cell = cellsArr[i * column + j]
  ...
  ContentState.prototype.copyHandler = function (event: ClipboardEvent, type: string, copyInfo = null) {
    if (this.selectedTableCells) {
    ...
    const { selectedImage } = this
    if (selectedImage) {
      const { token } = selectedImage as { token: { raw: string; [key: string]: unknown } }
    ...
    switch (type) {
      ...
      case 'copyBlock': {
        const block = typeof copyInfo === 'string' ? this.getBlock(copyInfo) : (copyInfo as Block | null)
        if (!block) return
