...
import type { IContentState, Block, Token, Cursor, ListItemType, ListType } from '../types'
...
const updateCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.checkSameMarkerOrDelimiter = (list: Block, markerOrDelimiter: string) => {
   ...
   */
  ContentState.prototype.checkInlineUpdate = function (block: Block) {
    ...
    const [_match, bullet, tasklist, order, atxHeader, setextHeader, blockquote, indentCode, footnote, hr] =
      text.match(INLINE_UPDATE_REG) || []
    const { footnote: isSupportFootnote } = this.muya.options

    switch (true) {
      case !!hr && new Set(hr.split('').filter((i: string) => /\S/.test(i))).size === 1:
        return this.updateThematicBreak(block!, hr as string, line!) as Block

      case !!bullet:
        return this.updateList(block!, 'bullet', bullet, line!) as Block

      // only `bullet` list item can be update to `task` list item
      case !!tasklist && listItem && listItem.listItemType === 'bullet':
        return this.updateTaskListItem(block!, 'tasklist', tasklist) as unknown as Block

      case !!order:
        return this.updateList(block!, 'order', order, line!) as Block

      case !!atxHeader:
        return this.updateAtxHeader(block!, atxHeader as string, line!) as Block

      case !!setextHeader:
        return this.updateSetextHeader(block!, setextHeader as string, line!) as Block

      case !!blockquote:
        return this.updateBlockQuote(block!, line!) as Block

      case !!indentCode:
        return this.updateIndentCode(block!, line!) as Block

      case !!footnote && block!.type === 'p' && !block!.parent && isSupportFootnote:
        return this.updateFootnote(block!, line!) as Block
      default:
        return this.updateToParagraph(block!, line!) as Block
    }
  }

  // Thematic break
  ContentState.prototype.updateThematicBreak = function (block: Block, _marker: string, line: Block) {
  ...
  ContentState.prototype.updateList = function (block: Block, type: string, marker = '', line: Block) {
    ...
    if (
      ...
      const listBlock = this.createBlock(wrapperTag, {
        listType: type as ListType,
      })

      if (wrapperTag === 'ol') {
        const startNum = cleanMarker ? cleanMarker.slice(0, -1) : '1'
        listBlock.start = /^\d+$/.test(startNum) ? startNum : 1
      }
  ...
  ContentState.prototype.updateTaskListItem = function (block: Block, _type: string, marker: string = '') {
    const { preferLooseListItem } = this.muya.options
  ...
  ContentState.prototype.updateAtxHeader = function (block: Block, header: string, line: Block) {
  ...
  ContentState.prototype.updateSetextHeader = function (block: Block, marker: string, line: Block) {
  ...
  ContentState.prototype.updateBlockQuote = function (block: Block, line: Block) {
    const text = line.text
    const lines = text.split('\n')
    const preParagraphLines = []
    const quoteLines = []
  ...
  ContentState.prototype.updateIndentCode = function (block: Block, line: Block) {
    const lang = ''
  ...
  ContentState.prototype.updateToParagraph = function (block: Block, line: Block) {
