...
const inputCtrl = (ContentState: { prototype: IContentState }) => {
  ...
  ContentState.prototype.inputHandler = function (event: InputEvent, notEqual = false) {
    ...
    const checkMarkedUpdate = /atxLine|paragraphContent|cellContent/.test(block!.functionType ?? '')
      ...
      : false
    let inlineUpdatedBlock: boolean | Block | null = null
    ...
    if (inlineUpdatedBlock && inlineUpdatedBlock !== true) {
      const liBlock = this.getParent(inlineUpdatedBlock as Block)
