import type { IContentState, CursorPosition, LineCursor } from '../types'

const coreApi = (ContentState: { prototype: IContentState }) => {
  /**
  ...
  ContentState.prototype.replaceWordInline = function (this: IContentState, line: LineCursor, wordCursor: { start: CursorPosition; end: CursorPosition }, replacement: string, setCursor = false) {
    ...
    if (setCursor) {
      ...
      line.start = cursor as typeof line.start
      line.end = cursor
      this.cursor = {
        start: cursor,
        end: cursor,
