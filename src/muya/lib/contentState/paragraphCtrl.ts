...
import type { IContentState, Block, SelectionCursorPos, ListType, ListItemType } from '../types'
...
const paragraphCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.selectionChange = function (cursor?: unknown) {
    const cursorArg = cursor as { start: SelectionCursorPos; end: SelectionCursorPos } | undefined
    const { start, end } = cursorArg || selection.getCursorRange()
    if (!start || !end) {
  ...
  ContentState.prototype.getCommonParent = function () {
    ...
    const startBlock = this.getBlock(start.key)
    const endBlock = this.getBlock(end.key)
    const startParentKeys = this.getParents(startBlock!).map((b: Block) => b.key)
    const endParentKeys = this.getParents(endBlock!).map((b: Block) => b.key)
  ...
  ContentState.prototype.handleListMenu = function (paraType: string, insertMode?: string) {
    ...
    const isListed = affiliation.slice(0, 3).filter((b: Block) => /ul|ol/.test(b.type))

    if (isListed.length && !insertMode) {
      const listBlock = isListed[0]
      if (listType === listBlock.listType) {
        const listItems = listBlock.children
        listItems.forEach((listItem: Block) => {
          listItem.children.forEach((itemParagraph: Block) => {
      ...
      if (start.key === end.key || (start.block?.parent && start.block?.parent === end.block?.parent)) {
        ...
        const paragraph = this.getBlock(block.parent as string)
        if (listType === 'task') {
          // 1. first update the block to bullet list
          const listItemParagraph = this.updateList(paragraph!, 'bullet', undefined, block) as Block
        ...
        listWrapper.listType = listType as ListType
        if (listType === 'order') listWrapper.start = 1

        children.slice(startIndex, endIndex + 1).forEach((child: Block) => {
          if (child !== referBlock) {
          ...
          listItem.listItemType = listType as ListItemType
  ...
  ContentState.prototype.handleQuoteMenu = function (insertMode?: string) {
    const { start, end, affiliation } = this.selectionChange(this.cursor)
    let startBlock = this.getBlock(start.key)
    const isBlockQuote = affiliation.slice(0, 2).filter((b: Block) => /blockquote/.test(b.type))
  ...
  ContentState.prototype.insertContainerBlock = function (functionType: string, block: Block) {
  ...
  ContentState.prototype.showTablePicker = function () {
    ...
    const handler = (rows: number, columns: number) => {
  ...
  ContentState.prototype.insertHtmlBlock = function (block: Block) {
    if (block.type === 'span') {
  ...
  ContentState.prototype.updateParagraph = function (paraType: string, insertMode = false) {
    ...
    switch (paraType) {
      ...
      case 'ul-task':
      case 'ol-order': {
        needDispatchChange = this.handleListMenu(paraType, insertMode ? 'true' : undefined)
        break
      }
      ...
      case 'blockquote': {
        this.handleQuoteMenu(insertMode ? 'true' : undefined)
        break
      }
      case 'mathblock': {
      ...
      case 'paragraph': {
        ...
        const startOffset = newLevel > 0 ? start.offset + newLevel - hash.length + 1 : start.offset - hash.length // no need to add `1`, because we didn't add `String.fromCharCode(160)` to text paragraph
        const endOffset = newLevel > 0 ? end.offset + newLevel - hash.length + 1 : end.offset - hash.length
        let newText =
          newLevel > 0
  ...
  ContentState.prototype.insertParagraph = function (location: string, text = '', outMost = false) {
    const { start, end } = this.cursor
  ...
  ContentState.prototype.getTypeFromBlock = function (block: Block) {
    const { type } = block
    ...
    switch (type) {
      case 'span': {
        if (block.functionType === 'atxLine') {
          ...
          if (rootBlock && rootBlock.functionType !== 'fencecode') {
            ...
            internalType = rootBlock.functionType as string
          } else {
            internalType = 'pre'
          }
        ...
        const listTypes = affiliation
          ...
          .filter((b: Block) => /ul|ol/.test(b.type))
          .map((b: Block) => b.listType)
      ...
      case 'figure': {
        if (block.functionType === 'multiplemath') {
          ...
          internalType = block.functionType as string
        }
        break
      }
      case 'pre': {
