...
const adjustOffset = (offset: number, block: Block, event: KeyboardEvent) => {
...
const arrowCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.findNextRowCell = function (this: IContentState, cell: Block) {
  ...
  ContentState.prototype.findPrevRowCell = function (this: IContentState, cell: Block) {
    if (cell.functionType !== 'cellContent') {
      throw new Error(`block with type ${cell?.type} is not a table cell`)
    }
    const thOrTd = this.getParent(cell)
  ...
  ContentState.prototype.docArrowHandler = function (this: IContentState, event: KeyboardEvent) {
    const { selectedImage } = this
    if (selectedImage) {
      const { key, token } = selectedImage as { key: string; token: { range: { start: number; end: number } } }
  ...
  ContentState.prototype.arrowHandler = function (this: IContentState, event: KeyboardEvent) {
    ...
    const { topOffset, bottomOffset } = selection.getCursorYOffset(paragraph as HTMLElement)
    if (!start || !end) {
      return
    }
    ...
    if (
      ...
      (node as HTMLElement).classList.contains(CLASS_OR_ID.AG_MATH_TEXT)
    ) {
    ...
    if (event.key === EVENT_KEYS.ArrowUp || (event.key === EVENT_KEYS.ArrowLeft && start.offset === 0)) {
      ...
      const offset = adjustOffset(0, (nextBlock || newBlock) as Block, event)
      this.cursor = {
        start: { key, offset },
        end: { key, offset },
      }
