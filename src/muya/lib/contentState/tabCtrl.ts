...
const tabCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.findNextCell = function (block: Block): Block | null {
  ...
  ContentState.prototype.findPreviousCell = function (block: Block): Block | null {
    if (block.functionType !== 'cellContent') {
  ...
  ContentState.prototype.isUnindentableListItem = function (block: Block) {
  ...
  ContentState.prototype.isIndentableListItem = function (): boolean {
  ...
  ContentState.prototype.checkCursorAtEndFormat = function (text: string, offset: number) {
    ...
    let result = null
    const walkTokens = (tkns: Token[]) => {
      for (const token of tkns) {
        const { marker, type, range, children, srcAndTitle, hrefAndTitle, backlash, closeTag, isFullLink, label } =
          token
        ...
        if (BOTH_SIDES_FORMATS.includes(type) && offset > start && offset < end) {
          switch (type) {
            ...
            case 'html_tag': {
              if (closeTag && offset === end - (closeTag as string).length) {
                result = {
                  offset: (closeTag as string).length,
                }
                return
              }
              break
    ...
    walkTokens(tokens as Token[])

    return result
  }

  ContentState.prototype.tabHandler = function (event: KeyboardEvent) {
    // disable tab focus
