...
const tableSelectCellsCtrl = (ContentState: { prototype: IContentState }) => {
  ContentState.prototype.handleCellMouseDown = function (event: MouseEvent) {
    if (event.buttons === 2) {
    ...
    const row = getIndex('left', cell as HTMLElement)
    const column = getIndex('bottom', cell as HTMLElement)
    this.cellSelectInfo = {
      tableId,
      anchor: {
        key: (cell as HTMLElement).id,
        row,
        column,
      },
      focus: null,
  ...
  ContentState.prototype.handleCellMouseMove = function (event: MouseEvent) {
    const target = event.target as HTMLElement
    const cell = target.closest('th') || target.closest('td')
    const table = target.closest('table')
    const isOverSameTableCell = cell && table && table.id === this.cellSelectInfo!.tableId
    if (isOverSameTableCell && (cell as HTMLElement).id !== this.cellSelectInfo!.anchor!.key) {
  ...
  ContentState.prototype.handleCellMouseUp = function (event: MouseEvent) {
    const { eventCenter } = this.muya
    for (const id of this.cellSelectEventIds) {
      eventCenter.detachDOMEvent(id)
    }
    this.cellSelectEventIds = []
    if (this.cellSelectInfo?.isStartSelect) {
      ...
      setTimeout(() => {
        this.selectedTableCells = {
          ...
          cells: selectedCells!.map((c: Block) => {
            delete c.ele
            return c
          }),
        }
  ...
  ContentState.prototype.deleteSelectedTableCells = function (isCut = false) {
    ...
    const row = tableBlock.row
    const column = tableBlock.column
    const rows = new Set()
    let lastColumn = null
  ...
  ContentState.prototype.selectTable = function (table: Block) {
    ...
    this.cellSelectInfo = {
      ...
      cells: getAllTableCells(table.key) as HTMLElement[][],
    }
    this.calculateSelectedCells()
    this.selectedTableCells = {
      tableId: table.key,
      row: (table.row ?? 0) + 1,
      column: (table.column ?? 0) + 1,
      cells: this.cellSelectInfo!.selectedCells!.map((c: Block) => {
        delete c.ele
        return c
      }),
    }
  ...
  ContentState.prototype.isWholeTableSelected = function () {
    ...
    const table = this.getBlock(selectedTableCells.tableId)
    if (!table) {
      return null
    }
    const row = table.row ?? 0
    const column = table.column ?? 0
    if (selectedTableCells.cells.length === (row + 1) * (column + 1)) {
      return table
    }
