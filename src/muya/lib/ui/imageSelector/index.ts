...
class ImageSelector extends BaseFloat {
  ...
  listen() {
    ...
    eventCenter.subscribe('muya-image-selector', (({ reference, cb, imageInfo }: { reference: HTMLElement | null; cb: (...args: unknown[]) => void; imageInfo: ImageInfo }) => {
      if (reference) {
        ...
        const input = this.imageSelectorContainer.querySelector('input.src') as HTMLInputElement | null
        if (input) {
          input.focus()
          input.select()
        }
  ...
  tabClick(_event: Event, tab: { value: string }) {
    const { value } = tab
    this.tab = value
    return this.render()
  }
  ...
  inputHandler(event: Event, type: keyof ImageState) {
  ...
  async handleKeyUp(event: KeyboardEvent) {
    ...
    const value = (event.target as HTMLInputElement).value
    const { eventCenter } = this.muya
    const reference = this.imageSelectorContainer.querySelector('input.src') as HTMLInputElement
    ...
    if (!this.muya.options.imageAction || URL_REG.test(src)) {
      ...
      if (src) {
        ...
        try {
          ...
          const imageWrapper = this.muya.container.querySelector(`span[data-id=${id}]`) as HTMLElement | null
  ...
  async handleSelectButtonClick() {
    ...
    const path = await (this.muya.options.imagePathPicker as () => Promise<string>)()
      ...
      return h(
        ...
        h(
          'span',
          {
            on: {
              click: (event) => {
                this.tabClick(event as Event, tab)
              },
            },
          },
          tab.label,
  ...
  renderBody = () => {
    ...
    if (tab === 'select') {
      ...
      const altInput = h('input.alt', {
        ...
        on: {
          input: (event) => {
            this.inputHandler(event as Event, 'alt')
          },
          paste: (event) => {
            this.inputHandler(event as Event, 'alt')
          },
      ...
      const srcInput = h('input.src', {
        ...
        on: {
          input: (event) => {
            this.inputHandler(event as Event, 'src')
          },
          paste: (event) => {
            this.inputHandler(event as Event, 'src')
          },
          keydown: (event) => {
            this.srcInputKeyDown(event as KeyboardEvent)
          },
          keyup: (event) => {
      ...
      const titleInput = h('input.title', {
        ...
        on: {
          input: (event) => {
            this.inputHandler(event as Event, 'title')
          },
          paste: (event) => {
            this.inputHandler(event as Event, 'title')
          },
      ...
      if (this.loading) {
        ...
        const photos = this.photoList.map((photo: UnsplashPhoto) => {
