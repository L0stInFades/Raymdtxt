...
class FrontMenu extends BaseFloat {
  ...
  renderSubMenu(subMenu: SubMenuItem[]) {
    const { reference } = this
    if (!reference) return h('div')
    const rect = reference.getBoundingClientRect()
    const windowHeight = document.documentElement.clientHeight
    const children = subMenu.map((menuItem: SubMenuItem) => {
      ...
      return h(
        itemSelector,
        {
          on: {
            click: (event) => {
              this.selectItem(event as Event, { label })
            },
          },
        },
  ...
  render() {
    ...
    if (!outmostBlock || !startBlock || !endBlock) return
