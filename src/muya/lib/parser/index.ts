...
const matchHtmlTag = (src: string, disableHtml: boolean) => {
...
export const tokenizer = (src: string, { highlights = [] as { start: number; end: number; active: boolean }[], hasBeginRules = true, labels = new Map<string, { href: string; title: string }>(), options = {} as Record<string, unknown> } = {}) => {
  ...
  const postTokenizer = (tokens: Record<string, unknown>[]) => {
    for (const token of tokens) {
      for (const light of highlights) {
        const highlight = union(token.range as { start: number; end: number }, light)
        if (highlight) {
...
export const generator = (tokens: { raw: string }[]) => {
  let result = ''
  for (const token of tokens) {
    result += token.raw
  }
