...
// biome-ignore lint/suspicious/noExplicitAny: legacy constructor-function pattern with dynamic token arrays
function Lexer(this: any, opts: Record<string, unknown>) {
  this.tokens = []
...
Lexer.prototype.token = function (src: string, top: boolean) {
  ...
  while (src) {
    ...
    if (cap) {
      ...
      for (; i < l; i++) {
        ...
        space = item.length
        let newBull = bull
        item = item.replace(/^ *([*+-]|\d+(?:\.|\))) {0,4}/, (_m: string, p1: string) => {
