...
const PRE_BLOCK_HASH: Record<string, string> = {
...
export default function renderContainerBlock(
  ...
  if (/^(?:th|td)$/.test(type)) {
    ...
    if (cells?.length) {
      const cell = cells.find((c: Record<string, unknown>) => c.key === key)
      if (cell) {
    ...
    if (/html|multiplemath|flowchart|mermaid|sequence|plantuml|vega-lite/.test(functionType as string)) {
    ...
    selector += PRE_BLOCK_HASH[block.functionType as string]

    if (/html|multiplemath|mermaid|flowchart|vega-lite|sequence|plantuml/.test(functionType as string)) {
      const codeBlock = (block.children as Record<string, unknown>[])[0] as Record<string, unknown>
      const code = (codeBlock.children as { text: string }[]).map((line: { text: string }) => line.text).join('\n')
